...
# ============================================================
# FEATURE ENGINEERING — DELAI
# ============================================================

# Transformation logarithmique
df["log_delai"] = np.log1p(
    df["delai_reservation_jours"]
)


# Catégorisation du délai
df["delai_tres_court"] = (
    df["delai_reservation_jours"] <= 7
).astype(int)

df["delai_court"] = (
    (df["delai_reservation_jours"] > 7) &
    (df["delai_reservation_jours"] <= 30)
).astype(int)

df["delai_moyen"] = (
    (df["delai_reservation_jours"] > 30) &
    (df["delai_reservation_jours"] <= 90)
).astype(int)

df["delai_long"] = (
    (df["delai_reservation_jours"] > 90) &
    (df["delai_reservation_jours"] <= 180)
).astype(int)
...
print("\n" + "=" * 60)
print("RESULTATS — FEATURES DELAI")
print("=" * 60)
