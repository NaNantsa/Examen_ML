...
df["personnes_par_chambre"] = (
    df["nombre_personnes"] /
    df["chambres"].replace(0, np.nan)
)


# Nombre de nuits par personne
df["nuits_par_personne"] = (
    df["nuits"] /
    df["nombre_personnes"].replace(0, np.nan)
)


# Montant total par personne
df["montant_par_personne"] = (
    df["montant_total_eur"] /
    df["nombre_personnes"].replace(0, np.nan)
)


# Montant total par chambre
df["montant_par_chambre"] = (
    df["montant_total_eur"] /
    df["chambres"].replace(0, np.nan)
)
...
# IMPORTANT :
# On utilise la date de réservation pour respecter
# l'ordre temporel du problème.
...
print("\n" + "=" * 60)
print("RESULTATS — FEATURES SEJOUR")
print("=" * 60)
